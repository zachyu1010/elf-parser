...
/*
 * print program header entries in the table
 */
void print_program_headers(int32_t fd, Elf32_Ehdr eh, Elf32_Phdr ph_table[])
{
    uint32_t i;

    printf("<<<<< print program header >>>>>\n");
    printf("========================================");
    printf("========================================\n");
    printf(" Type    Offset     VirtAddr   PhysAddr   FSiz MemSiz     Flg        Align\n");
    printf("========================================");
    printf("========================================\n");
    ...
    for(i=0; i<eh.e_phnum; i++) {
        if(ph_table[i].p_type <= 7) {
            printf(" %s ", p_type_name[ph_table[i].p_type]);
        } else {
            printf(" %s ", "UNKNOWN");
        }
...
void save_text_section(int32_t fd, Elf32_Ehdr eh, Elf32_Shdr sh_table[])
{
    ...
    printf("<<<<< save binary text section >>>>>\n");
    char *pwd = getcwd(NULL, (size_t)NULL);
    printf("%s\n", pwd);
    pwd = realloc(pwd, strlen(pwd)+8);
    strcat(pwd,"/text.S");
